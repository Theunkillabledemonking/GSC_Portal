...
exports.googleLogin = async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT * FROM users WHERE eamil = ?', [googleUser.email]);
...
exports.registerUser = async (req, res) => {
    try {
        // 1. 요청 본문에서 사용자 정보를 추출합니다.
        const { email, name, student_id, phone, grade, level } = req.body;

        // 2. MySQL 데이터베이스에 사용자 정보를 삽입합니다.
        // 기본값:
        // -  role: 3 (학생)
        // -  status: 0 (승인 대기)
        await pool.query(
            'INSERT INTO users (eamil, name, student_id, phone, grade, level, role, status) VALUES (?, ?, ?, ?, ?, ?, 3, 0)',
            [email, name, student_id, phone, grade, level]
        );

        // 3. 사용자 등록이 완료ㅕ되면 201 상태 코드와 메시지를 반환합니다.
        res.status(200).json({ message: '등록 완료. 승인 대기 중입니다.'});
    } catch (error) {
        // 4. 오류 발생 시, 500 상태 코드와 오류 메시지를 반환합니다.
